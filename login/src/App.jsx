...
export default function LoginForm() {
  ...
  const handleSubmit = () => {
    ...
    .then(() => {
      alert('Deu bom cumpadi!')
      setEmail('')
      setPassword('')
    })
    .catch((error) => {
      setError(error.message)
    }) //then ativado quando retorna sucesso e vice-versa.
    .finally(() => { // vai ser executada sendo then ou catch (dando certo ou não)
    ...
    <div className='wrapper'>
      <div className='login-form'>
        ...
        <div className='row'>
          ...
          <input id={'email'} type={'email'} autoComplete='off' value={email} onChange={handleChangeEmail}/>
        </div>
        <div className='row'>
